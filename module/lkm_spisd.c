...
#define LKM_SPISD_MAJOR 222
#define LKM_SPISD_NR_DEVS 1
#define LKM_SPISD_DEV_NAME "lkm_spisd"
...
static int lkm_spisd_open(struct inode *inode, struct file *filp);
static int lkm_spisd_release(struct inode *inode, struct file *filp);
static int lkm_spisd_read(struct file *filp, char __user *buf, size_t count, loff_t *f_ops);
static int lkm_spisd_write(struct file *filp, char const __user *buf, size_t count, loff_t *f_ops);

static struct file_operations lkm_spisd_fops = {
	.owner = THIS_MODULE,
	.open = lkm_spisd_open,
	.release = lkm_spisd_release,
	.read = lkm_spisd_read,
	.write = lkm_spisd_write,
};
